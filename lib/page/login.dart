...
import 'package:ps_159/page/show_product.dart';
...
class LoginScreen extends StatelessWidget {
  ...
  Future<String?> _authUser(LoginData data) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://642021159.pungpingcoding.online/api/login'),
        body: {
          'email': data.name,
  ...
  Future<String?> _signupUser(SignupData data) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://642021159.pungpingcoding.online/api/register'),
        headers: {
  ...
  Widget build(BuildContext context) {
    return FlutterLogin(
      // title: 'PK SHOP',
      theme: LoginTheme(
        primaryColor: Color.fromARGB(255, 203, 196, 135),
        accentColor: Colors.white,
        pageColorDark: const Color.fromARGB(255, 255, 255, 255),
        pageColorLight: Color.fromARGB(255, 203, 196, 135),
        authButtonPadding: const EdgeInsets.only(top: 30, bottom: 30),
        switchAuthTextColor: Color.fromARGB(255, 121, 32, 126),
        footerTextStyle: TextStyle(color: Color.fromARGB(255, 121, 32, 126)),
        // bodyStyle: TextStyle(fontWeight: FontWeight.bold),
        
        
      ),
      logo: const AssetImage('assets/images/ps.png'),
      onLogin: _authUser,
